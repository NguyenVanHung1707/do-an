...
\begin{itemize}
    ...
    \item \textbf{Khả năng tích hợp linh hoạt}: Keycloak cung cấp bộ Admin Client cho Java/Spring Boot, giúp backend dễ dàng thực hiện các tác vụ quản lý tài khoản như phê duyệt giảng viên mới, kích hoạt/khóa tài khoản và đồng bộ dữ liệu người dùng.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Khả năng tương tác phần cứng tốt}: React Native cung cấp các cầu nối native (Native Bridges) ổn định để truy cập trực tiếp vào phần cứng thiết bị như cảm biến định vị GPS, camera thu thập ảnh Face ID và phân hệ bảo mật của hệ điều hành để lưu khóa mã hóa vân tay/Face ID.
