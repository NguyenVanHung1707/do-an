...
import { LogIn, UserPlus, ShieldAlert, KeyRound, BookOpen, Clock } from 'lucide-react';
        ...
        <div className="md:col-span-7 p-6 md:p-10 flex flex-col justify-center bg-white dark:bg-slate-900">
          {showTeacherSuccess ? (
            <div className="flex flex-col items-center text-center py-6 justify-center">
              <div className="w-16 h-16 bg-amber-50 dark:bg-amber-950/35 text-amber-500 dark:text-amber-400 rounded-full flex items-center justify-center mb-6 shadow-md border border-amber-100 dark:border-amber-900/50 animate-bounce">
                <Clock className="w-8 h-8" />
              </div>
              
              <h1 className="font-headline text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3 tracking-tight">
                Đăng ký thành công!
              </h1>
              
              <p className="font-inter text-sm text-slate-600 dark:text-slate-350 max-w-md leading-relaxed mb-6">
                Yêu cầu tạo tài khoản giảng viên của bạn (Mã số: <code className="bg-slate-100 dark:bg-slate-850 px-1.5 py-0.5 rounded font-mono font-bold text-slate-700 dark:text-slate-300">{code}</code>) đã được tạo thành công trên hệ thống. Vui lòng chờ <strong>Quản trị viên (Admin)</strong> phê duyệt tài khoản trước khi đăng nhập.
              </p>

              <div className="w-full max-w-md bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl border border-slate-100 dark:border-slate-850 text-left mb-8 space-y-3">
                <div className="flex items-start gap-3">
                  <div className="w-1.5 h-1.5 bg-primary rounded-full mt-2 shrink-0" />
                  <p className="text-xs text-slate-655 dark:text-slate-400">
                    Kết quả phê duyệt tài khoản sẽ được gửi tự động về hòm thư Email đăng ký của bạn (<strong className="text-slate-850 dark:text-slate-200">{email}</strong>).
                  </p>
                </div>
                <div className="flex items-start gap-3">
                  <div className="w-1.5 h-1.5 bg-primary rounded-full mt-2 shrink-0" />
              ...
              <button
                onClick={() => {
                  setShowTeacherSuccess(false);
                  setIsRegister(false);
                }}
                className="w-full max-w-xs h-[42px] flex items-center justify-center gap-2 bg-primary hover:bg-primary-hover active:bg-primary-active text-white rounded-md font-bold shadow-subtle hover:shadow-medium transition-all duration-150"
              >
